...
//            System.out.println("heelll333");
    ...
    public static void moveStone2(String[] boardGame, String stone, int steps) {
        // Find the current index of the stone
        int currentIndex = -1;
        for (int i = 0; i < boardGame.length; i++) {
            if (boardGame[i] != null && boardGame[i].contains(stone)) {
        ...
        if (steps > remainingStepsToEnd) {
            ...
            if(steps==24||steps==10) {
                ...
                int num = input.nextInt();
                if (num == 1) {
                    Movement.chooseStoneToMove2(boardGame, 1);
                } else if (num == 2) {
                    BoardGame.insertStone2(boardGame, 1);
                }
            }
            System.out.println("press 1 to  choose another stone to move or 2 to throw again");
            int num1 = input.nextInt();
            if (num1 == 1) {
                chooseStoneToMove2( boardGame, steps);
            } else if (num1 == 2) {
                return;
            }
...
//            System.out.println("heelll333");
    ...
    public static void chooseStoneToMove2(String[] boardGame, int steps) {
        Scanner scanner = new Scanner(System.in);

        // Display the "H" stones in the board game
        System.out.println("B stones in the board game:");
        for (int i = 0; i < boardGame.length; i++) {
            if (boardGame[i] != null && boardGame[i].contains("B")) {
                System.out.println("Index: " + i + ", Stone: " + boardGame[i]);
            }
        }
        ...
        // Move the chosen stone
        moveStone2(boardGame, index, steps);
